		...
		<c:forEach items="${page.list}" var="user">
			<tr>
				...
				<shiro:hasPermission name="sys:user:edit"><td>
					...
					<a href="${ctx}/sys/user/delete?id=${user.id}&flag=0" onclick="return confirmx('确认要禁用该用户吗？', this.href)">禁用</a>
					</c:if>
					<c:if test="${user.startFlag=='1' }">
					<a href="${ctx}/sys/user/delete?id=${user.id}&flag=1" onclick="return confirmx('确认要启动该用户吗？', this.href)">启动</a>
					</c:if>
