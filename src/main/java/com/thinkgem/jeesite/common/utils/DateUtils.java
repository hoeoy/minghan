...
//		System.out.println(time/(24*60*60*1000));
	...
	private static long HOUR_MS=60*60*1000;
	private static SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
	public static String getDefaultTime(Date time){
		if(time != null){
			try {
				Date date = sdf.parse(sdf.format(new Date()));
				String comTime = sdf.format(time);
				Date compareTime = sdf.parse(comTime);
				SimpleDateFormat msdf = null;
				if(date.getTime()-compareTime.getTime()>24*HOUR_MS){
					msdf = new SimpleDateFormat("yyyy/MM/dd HH:mm");
					return msdf.format(time);
				}
				if(date.getTime()-compareTime.getTime()==24*HOUR_MS){
					return "昨天";
				}
	...
	public static String getDefaultTime2(Date time){
		if(time != null){
			try {
				Date date = sdf.parse(sdf.format(new Date()));
				String comTime = sdf.format(time);
	...
	public static String getDefaultTime3(Date time){
		if(time != null){
			try {
				Date date = sdf.parse(sdf.format(new Date()));
				String comTime = sdf.format(time);
